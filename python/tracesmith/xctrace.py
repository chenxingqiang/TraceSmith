...
try:
    from . import EventType, TraceEvent, export_perfetto
except ImportError:
...
class MetalGPUEvent:
    ...
    def to_trace_event(self) -> "TraceEvent":
        ...
        event.correlation_id = (
            int(self.gpu_submission_id, 16)
            if self.gpu_submission_id.startswith("0x")
            else int(self.gpu_submission_id)
        )
        event.device_id = 0
...
class XCTraceProfiler:
    ...
    def is_available() -> bool:
        ...
        try:
            result = subprocess.run(
                ["xcrun", "xctrace", "version"], capture_output=True, text=True, timeout=5
            )
    ...
    def get_metal_templates() -> List[str]:
        ...
        return [t for t in all_templates if any(kw.lower() in t.lower() for kw in metal_keywords)]

    def profile_command(
        self, command: List[str], duration: Optional[int] = None, output_file: Optional[str] = None
    ) -> List["TraceEvent"]:
        """
        ...
        print("Starting xctrace profiling...")
    ...
    def _parse_trace(self, trace_path: str) -> List["TraceEvent"]:
        ...
        gpu_events = self._export_table(trace_path, "metal-gpu-execution-points")
        ...
        driver_events = self._export_table(
            trace_path, "metal-driver-intervals", target_pid="SINGLE"
        )
    ...
    def _export_table(
        self, trace_path: str, schema: str, target_pid: Optional[str] = None
    ) -> List[Any]:
    ...
    def _parse_gpu_execution_points(self, root: ET.Element) -> List[MetalGPUEvent]:
        ...
        for row in root.findall(".//row"):
            try:
                ...
                gpu_submission_id = (
                    submission_elem[1].get("fmt", "0") if len(submission_elem) > 1 else "0"
                )
    ...
    def _parse_driver_intervals(self, root: ET.Element) -> List[MetalDriverInterval]:
        ...
        for row in root.findall(".//row"):
            try:
                ...
                event = MetalDriverInterval(start_ns=start_ns, duration_ns=duration_ns, name=name)
                events.append(event)
...
def add_xctrace_arguments(parser):
    ...
    parser.add_argument(
        "--keep-trace", action="store_true", help="Keep the raw .trace file after profiling"
    )
...
if __name__ == "__main__":
    ...
    events, trace_file = profile_with_xctrace(
        ["python3", "-c", "print('Hello from xctrace test')"], duration=3
    )
