...
def profile_pytorch_metax():
    ...
    try:
        ...
        print(
            f"Device name: {torch.cuda.get_device_name(0) if hasattr(torch.cuda, 'get_device_name') else 'Unknown'}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="MetaX GPU Profiling Example")
    parser.add_argument(
        "--mode",
        "-m",
    ...
    parser.add_argument(
        "--output", "-o", default="metax_trace", help="Output file prefix (default: metax_trace)"
    )
